...
<br><br>
첫 번째 트러블 슈팅과 비슷한 상황(자바스크립트 문제)은 여러번 발생했었다. 그리고 여러번 오류의 원인과 문제를 해결해가면서 처음부터 어떻게 데이터를 받고 어떻게 뿌릴 것인지 정확히 알고, 쪼개고, 하나씩 해야 한다는 것을 깨달았고, 꼼꼼하게 출력을 통해 확인하면서 작업을 진행해야 함을 절실히 깨달았다. 두 번째 트러블 슈팅을 통해서는 Redirect(브라우저가 새로운 요청 수행), Forward(같은 요청 안에서 화면 전환), 쿼리스트링(URL 뒤에 key=value로 데이터 전달)의 개념을 명확히 이해했다. 향후 비슷한 문제가 생기면 더욱 신속히 해결할 수 있을 것이라고 확신하며, 이를 계기로 웹 개발에서 데이터 전달 방식에 대한 이해를 더욱 심화할 계획이다. 
