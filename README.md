...
알림 컨트롤러에서는 '마이페이지?category=alarm'으로 Redirect하도록 하였다. 해당 요청을 받으면 type값으로 알림 화면을 실행하도록 하였다. <br>
(상세 방법: 쿼리스트링에서 사용하는 key와 value값을 category와 alarm이라고 정하였고, <br>알림 컨트롤러에 해당 쿼리스트링과 함께 Redirect문을 작성하였다. <br>
마이페이지 컨트롤러에서 응답하는 forward문을 작성하였다. 그리고 model객체에 type 매개변수를 주입하였다. <br>자바스크립트에서는 type 값을 활용하여 조건에 따라 알림 화면을 표시하였다. 즉, 'if (type === "alarm") { 화면표시 실행코드 }'을 작성하였다.)<br>
